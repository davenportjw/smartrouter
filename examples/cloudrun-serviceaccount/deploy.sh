...
echo -e "${BLUE}=======================================================${NC}"
echo -e "${BLUE}🚀 Deploying Gemini Service Account Client to Cloud Run${NC}"
echo -e "${BLUE}=======================================================${NC}"
...
if [ -z "$ROUTER_URL" ]; then
  echo -e "${RED}❌ Error: ROUTER_URL is required for deployment.${NC}"
  exit 1
fi
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-env-vars="ROUTER_URL=$ROUTER_URL"
...
echo -e "  -d '{\"prompt\": \"Why is OIDC token authentication superior to static API keys?\"}'${NC}\n"
